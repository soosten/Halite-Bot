...
def decide(state, actor):
    ...
    if actor in state.my_ships:
        ...
        if "CONVERT" in no_self_col:  # same as "CONVERT" being legal
            if should_convert(actor, state, rates, time_to_yard):
        ...
        # decide on choosing moves from no_opp_col or no_self_col depending on
        # how conservative we want to be
        no_opp_col = [action for action in no_self_col
                      if not state.opp_collision(actor, action)]

        # this quantile needs to not contain the quantile that hunts!
        if ship_hal > np.quantile(state.my_ship_hal, 1/20):
            candidates = no_opp_col
        else:
            candidates = no_self_col

        # now check revert to no_self_col and legal as necessary when there
        # are no conservative moves one can make
        if len(candidates) == 0:
            candidates = no_self_col
        if len(candidates) == 0:
            candidates = legal

        # if len(no_self_col) == 0:
        #     print(f"Non-collision impossible at step {1 + state.step}")
        #     no_self_col = legal
...
# needs more work
def should_convert(actor, state, rates, time_to_yard):
    ship_pos, ship_hal = state.my_ships[actor]
    ...
    # do build a yard if there is not yard in our cluster
...
def should_spawn(actor, state, rates):
    ...
    # if state.config.episodeSteps - state.step < 20:
    #     return False
...
def ships_wanted(state, rates):
    ...
    #         + rates["my_space"] / 13
    S = (rates["free_space"] + rates["my_space"]) / 13
    # M = 15 if state.step < 350 else 10
    S = max(10, S)
    return S
...
def set_rates(actor, state):
    ...
    # keep a minimum rate of 2% even if we're dominating to keep depositing
    rates["risk"] = 0.02 + risk
    # print(rates["risk"])
...
def make_map(actor, state, weights, rates):
    ...
    if ahal < np.quantile(state.my_ship_hal, 0.95):
...
class State:
    ...
    def _make_clusters(self):
        ...
        model = OPTICS(min_samples=3, min_cluster_size=5, metric="precomputed")
...
def agent(obs, config):
    ...
    tock = time()
    while queue.not_empty():
        # agent has to make a decision in < 2 seconds
        if tock > tick + 1.9:
