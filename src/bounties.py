...
class Bounties:
    ...
    def set_ship_targets(self, state, ship_target_memory):
        ...
        # REMOVE
        self.new_targets_set = num_new_targets
    ...
    def get_ship_targets(self, ship, state):
        ...
        # we put slightly higher bounties on the 4 sites adjacent to
        # the ship as well so that ships collapse on the target
        # put bounties on the 5 sites around a ship - put slightly lower
        # value on the actual ship, unless there are 4 ships around it
        # in which case one should move on
    ...
    def get_yard_targets(self, ship, state):
        full_pos = np.array([], dtype=int)
        full_rew = np.array([], dtype=int)
        ...
        attack = (state.total_steps - state.step) < STEPS_FINAL
        attack = attack and (state.my_ship_pos.size > 5 + max_opp_ships)
        attack = attack and (hal == 0)
        ...
        closest = min(state.opp_ids, key=score_diff, default=None)

        if closest is not None:
            full_pos = state.opp_data[closest][1]
            full_rew = np.full_like(full_pos, 2 * state.max_halite)
